...
// Free tier: https://jsonblob.com — each blob holds up to 1MB

let localCache = null; // in-memory cache for current instance
...
async function loadDb() {
  ...
  // Initialize fresh
  localCache = { keys: {}, byEmail: {}, byPrefix: {}, billing: {} };
  return localCache;
}
...
app.get('/health', (req, res) => {
  res.status(200).json({
    status: 'ok', version: '3.5.0', timestamp: new Date().toISOString(),
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: { 'GET /health': 'Health check', 'POST /v1/signup': 'Create account', 'GET /v1/models': 'List models', 'POST /v1/chat/completions': 'Chat', 'GET /v1/usage': 'Usage stats', 'POST /v1/stripe/webhook': 'Stripe' },
...
// ── 404 ───────────────────────────────────────────────────────────────────
app.use((req, res) => apiError(res, 400, `Route ${req.method} ${req.path} not found`, 'not_found'));
