/**
 * NiceGuyAPI v5.2.1 - AI Agent Gateway with Custom Agents & Platinum Tier (Vercel Serverless)
 *
...
// Health
app.get('/health', (req,res) => res.status(200).json({ status:'ok', version:'5.2.1', timestamp:new Date().toISOString(), storage:process.env.JSONBLOB_ID?'persistent':'memory-only', agent:true, custom_agents:true, stripe:!!stripe }));

// Root
app.get('/', (req,res) => res.json({ name:'NiceGuyAPI', version:'5.2.1', description:'AI Model Gateway + Agent. Custom Agents. Stripe + key management. Platinum tier now available.', base_url:'/v1', pricing:{free:'$0/12req/75k',pro:'$6/40req/145k',premium:'$27/250req/315k',platinum:'$55/99999/750k'}, auth:'X-API-Key header', endpoints:['GET /health','POST /v1/signup','GET /v1/models','POST /v1/chat/completions','POST /v1/agent','GET /v1/usage','GET /v1/keys','POST /v1/keys','DELETE /v1/keys/:prefix','POST /v1/keys/:prefix/rotate','POST /v1/agent/reset','GET /v1/agents','POST /v1/agents','GET /v1/agents/:id','PUT /v1/agents/:id','DELETE /v1/agents/:id','POST /v1/agents/:id/reset'] }));
